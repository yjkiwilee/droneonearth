...
class GroundGeometry extends THREE.PlaneBufferGeometry {
    ...
    raycastLatLong(lat, long) {

        var lt = lat - this.lat + 1/2;
        var lng = long - this.long + 1/2;
        
        // ! potential outOfBounds below
        var xIdx = Math.floor(lng * RES_GEO);
        var yIdx = Math.floor((1 - lt) * RES_GEO);
        var idx = yIdx * RES_GEO + xIdx;
        
        var vertices = [];
        var origin = this.attributes.position.array;
        var partWidthSeg = 3;
        var partHeightSeg = 3;
        ...
        var coord = this.latlongToCoord(lat, long);
        ...
        return raycaster.intersectObject(msh)[0].point;
    }

    raycastXY(x, y) {
        var latLong = this.coordToLatlong(x, y);
        return this.raycastLatLong(latLong[0], latLong[1]);
    }
}